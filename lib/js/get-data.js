...
FHIR.oauth2.ready().then(function(client) {
  ...
  .then(function(data) {
    if (!data.entry || !data.entry.length) {
      ...
      displayMedication("No medications found.");
    } else {
  ...
  function addWeightAnnotation() {
    ...
    var ret = client.request({
        url: "Observation/" + recent_weight_id,
        method: "PATCH",
        body: "{'note': {'authorString': 'test author', 'text': '" + annotation + "'}}"
    })
    console.log(ret);
    var annotation_text = "";
    annotation_text += annotation;
    annotation_text += " [author]";
    annotation_text += " [date]";
    displayAnnotation(annotation_text);
  }
